...
-- MEJORA: se agregaron confidence_score y entropy_score para trazabilidad
--   confidence_score : puntuación 0.0-1.0 calculada por el detector (entropía + contexto + tipo de archivo)
--   entropy_score    : entropía de Shannon del valor detectado (mide aleatoriedad real de la cadena)
CREATE TABLE credentials_detected (
    ...
    confidence_score FLOAT DEFAULT 0.0,   -- Score de confianza (0.0 - 1.0)
    entropy_score    FLOAT DEFAULT 0.0    -- Entropía de Shannon del valor detectado
);
...
-- Índice para filtrar por confianza alta rápidamente
CREATE INDEX idx_credential_confidence ON credentials_detected(confidence_score);

-- Vista para análisis rápido de repositorios
...
SELECT
    ...
    AVG(cd.confidence_score)       AS avg_confidence_score  -- NUEVO
FROM repositories r
...
-- Vista para commits con credenciales (incluye nuevos campos de scoring)
...
SELECT
    ...
    cd.file_path,
    cd.severity,
    cd.confidence_score,    -- NUEVO
    cd.entropy_score,       -- NUEVO
    c.risk_score
FROM commits c
INNER JOIN repositories r          ON c.repo_id   = r.repo_id
INNER JOIN credentials_detected cd ON c.commit_id = cd.commit_id
ORDER BY cd.confidence_score DESC, c.commit_date DESC;  -- Ordenar por confianza primero
...
COMMENT ON TABLE credentials_detected IS 'Almacena credenciales expuestas detectadas con scoring de confianza';
...
COMMENT ON COLUMN credentials_detected.confidence_score IS 'Score 0.0-1.0: combina tipo de credencial, entropía, contexto y tipo de archivo';
COMMENT ON COLUMN credentials_detected.entropy_score    IS 'Entropía de Shannon del valor detectado. Valores reales suelen ser > 3.5';

-- Tabla para los resultados del Anomaly Detector
...
-- Tabla para los resultados de Clustering
CREATE TABLE repo_clusters (
    cluster_id SERIAL PRIMARY KEY,
    repo_id INTEGER REFERENCES repositories(repo_id) ON DELETE CASCADE,
    assigned_cluster VARCHAR(50),
