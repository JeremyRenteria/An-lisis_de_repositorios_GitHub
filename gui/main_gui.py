...
from models.ml_anomaly_detector import ml_anomaly_detector
from models.ml_repo_clusterer import ml_repo_clusterer
import pandas as pd
...
class GitHubAnalyzerGUI:
    ...
    def setup_style(self):
        ...
        style.configure('Sidebar.TButton',
            ...
            background=self.colors['bg_sidebar'], foreground=self.colors['text'])

        style.configure('Dark.TLabel',
            background=self.colors['bg_dark'], foreground=self.colors['text'],
            font=('Segoe UI', 10))
        style.configure('DarkTitle.TLabel',
            background=self.colors['bg_dark'], foreground=self.colors['accent'],
            font=('Segoe UI', 18, 'bold'))

        style.configure("Treeview",
            background="#2e2e3e", foreground="white",
            fieldbackground="#2e2e3e", font=('Segoe UI', 9), rowheight=25)
        style.map("Treeview",
            background=[('selected', self.colors['accent'])],
            foreground=[('selected', 'black')])
        style.configure("Treeview.Heading",
            background="#11111b", foreground=self.colors['accent'],
    ...
    def create_widgets(self):
        ...
        for text, screen in nav_items:
            btn = tk.Button(self.sidebar, text=text,
                            bg=self.colors['bg_sidebar'], fg=self.colors['text'],
    ...
    def create_analysis_screen(self):
        ...
        input_frame = tk.LabelFrame(screen, text=' Configuración de Análisis ',
        ...
        def create_field(row, label_text, default=''):
            ...
            entry = tk.Entry(input_frame, bg='#2e2e3e', fg='white',
                             insertbackground='white', bd=1, relief='flat', width=35)
        ...
        self.analyze_btn = tk.Button(
            input_frame, text="🚀 INICIAR ESCANEO",
        ...
        log_frame = tk.LabelFrame(screen, text=' Terminal de Operaciones ',
                                  bg=self.colors['bg_dark'], fg=self.colors['accent'],
        ...
        self.log_text = scrolledtext.ScrolledText(
            ...
            font=('Consolas', 10), insertbackground='white', bd=0, padx=10, pady=10)
        self.log_text.pack(fill='both', expand=True)

    def create_results_screen(self):
        """Crea la pantalla de resultados — incluye columna Confianza"""
        screen = tk.Frame(self.main_container, bg=self.colors['bg_dark'])
        ...
        # MEJORA: se agregó columna "Confianza" para mostrar el confidence_score del detector
        columns = ('ID', 'Tipo', 'Archivo', 'Línea', 'Severidad', 'Confianza', 'Commit SHA', 'Validez ML', 'Fecha')
        ...
        col_widths = {
            'ID': 50, 'Tipo': 130, 'Archivo': 200, 'Línea': 55,
            'Severidad': 80, 'Confianza': 80, 'Commit SHA': 80,
            'Validez ML': 160, 'Fecha': 130
        }
        for col in columns:
    ...
    def create_stats_screen(self):
        ...
        for _, (label, key, color) in enumerate(kpis):
            card = tk.Frame(self.kpi_frame, bg=color, bd=1, relief='flat', padx=20, pady=15)
            card.pack(side='left', expand=True, fill='both', padx=5)
            tk.Label(card, text=label, bg=color,
                     font=('Segoe UI', 9, 'bold'), fg=self.colors['accent']).pack()
            val_lb = tk.Label(card, text='0', bg=color,
                              font=('Segoe UI', 22, 'bold'), fg=self.colors['text'])
            val_lb.pack()
        ...
        self.left_chart_frame = tk.LabelFrame(
            self.charts_container, text=' Gravedad de Amenazas ',
        ...
        self.right_chart_frame = tk.LabelFrame(
            self.charts_container, text=' Radar por Proyectos ',
            bg=self.colors['bg_dark'], fg=self.colors['text'], font=('Segoe UI', 10, 'bold'))
        self.right_chart_frame.pack(side='left', fill='both', expand=True, padx=5)

        tk.Button(screen, text='🔄 REFRESCAR DASHBOARD',
                  bg=self.colors['accent'], fg=self.colors['bg_dark'],
    ...
    def create_ml_screen(self):
        ...
        concept_frame = tk.LabelFrame(
            screen, text=' Algoritmo de Decisión ',
            bg=self.colors['bg_dark'], fg=self.colors['accent'],
            font=('Segoe UI', 10, 'bold'), padx=20, pady=15)
        concept_frame.pack(fill='x', padx=20, pady=10)

        txt = ("El analizador utiliza un Árbol de Decisión CART para validar si un fragmento "
               "detectado por Regex es una credencial real o un falso positivo, basándose en "
               "el contexto del commit, la entropía de Shannon y el score de confianza del detector.")
        ...
        for _, (lbl, key) in enumerate([('Precisión (Accuracy):', 'accuracy'), ('Score F1:', 'f1')]):
            f = tk.Frame(metrics_frame, bg='#24273a', padx=20, pady=10)
            f.pack(side='left', expand=True, fill='both', padx=5)
            tk.Label(f, text=lbl, bg='#24273a', fg=self.colors['accent'],
                     font=('Segoe UI', 10, 'bold')).pack()
            v = tk.Label(f, text='0.0000', bg='#24273a', fg='white',
        ...
        def cyber_btn(text, cmd):
            return tk.Button(btn_frame, text=text,
                             bg=self.colors['accent'], fg=self.colors['bg_dark'],
        ...
        detail_frame = tk.LabelFrame(
            screen, text=' Análisis de Contexto ',
            bg=self.colors['bg_dark'], fg=self.colors['accent'],
            font=('Segoe UI', 10, 'bold'), padx=10, pady=10)
        detail_frame.pack(fill='both', expand=True, padx=20, pady=10)

        self.ml_detail_text = scrolledtext.ScrolledText(
            detail_frame, bg='#11111b', fg='#f5c2e7', font=('Consolas', 10), bd=0)
    ...
    def create_status_bar(self):
        ...
        self.status_bar = tk.Label(
            ...
            bg=self.colors['bg_sidebar'], fg=self.colors['accent'],
            font=('Segoe UI', 9), relief=tk.FLAT, anchor='w', padx=10, pady=5)
    ...
    def log(self, message):
        """Agrega un mensaje al log (seguro para hilos)"""
    ...
    def update_status(self, message):
        """Actualiza la barra de estado (seguro para hilos)"""
        def _update():
    ...
    def run_analysis(self, owner, repo, branch, max_commits):
        ...
        try:
            ...
            self.log(f"  Lenguaje: {repo_info['language']} | Estrellas: {repo_info['stars']}")

            repo_id = db_manager.insert_repository(
                repo_info['name'], repo_info['owner'], repo_info['url'])
            ...
            for idx, commit in enumerate(commits, 1):
                self.log(f"  [{idx}/{len(commits)}] {commit['sha'][:7]}...")

                commit_details = github_analyzer.get_commit_details(owner, repo, commit['sha'])
                ...
                commit_details['has_credentials'] = has_credentials
                commit_details['num_credentials'] = len(credentials_found)

                risk_score = ml_anomaly_detector.calculate_risk_score(commit_details)
                ...
                # CORRECCIÓN: pasamos file_paths como lista para que ml_classifier
                # pueda calcular is_sensitive_file correctamente
            ...
            db_manager.update_repository_stats(repo_id, len(commits), total_credentials, risk_level)

            self.log(f"\n{'='*60}")
            self.log(f"✓ ANÁLISIS COMPLETADO")
            self.log(f"  Commits: {len(commits)} | Credenciales: {total_credentials} | Riesgo: {risk_level}")
            self.log(f"{'='*60}\n")

            self.root.after(0, self.load_results)
            self.root.after(0, self.update_statistics)
            self.root.after(0, messagebox.showinfo, "Completado",
                            f"Commits analizados: {len(commits)}\nCredenciales detectadas: {total_credentials}")

        except Exception as e:
            self.log(f"\n✗ ERROR: {str(e)}")
            import traceback
            self.log(traceback.format_exc())
            self.root.after(0, messagebox.showerror, "Error", f"Error durante el análisis: {str(e)}")
    ...
    def determine_risk_level(self, total_credentials, total_commits):
        """Determina el nivel de riesgo del repositorio usando K-Means"""
        # Obtenemos las métricas promediadas hasta el momento si es posible
        # Para simplificar en tiempo real usamos el clusterer con stats básicas
        repo_stats = {
            'total_commits': total_commits,
            'credentials_count': total_credentials
        }
        return ml_repo_clusterer.get_risk_level(repo_stats)

    def extract_commit_features(self, commit, commit_details):
        """
        Extrae características de un commit para ML.

        CORRECCIÓN: ahora incluye 'file_paths' como lista de nombres de archivos
        del commit, para que ml_classifier pueda calcular is_sensitive_file
        correctamente (antes usaba 'file_path' que no existía en commits).
        """
        ...
        max_severity = max(
            (severity_map.get(d['severity'], 0) for d in regex_detections), default=0)

        # Lista correcta de archivos del commit
        file_paths = [f['filename'] for f in commit_details.get('files', [])]

        is_sens_file = any(
            credential_detector.is_sensitive_file(fp) for fp in file_paths)
        ...
        return {
            ...
            'has_config_files':        any(
                credential_detector.is_sensitive_file(fp) for fp in file_paths),
            'has_env_files':           any('.env' in fp.lower() for fp in file_paths),
    ...
    def load_results(self):
        ...
        MEJORA: ahora muestra la columna 'Confianza' (confidence_score)
        generada por el detector mejorado, formateada como porcentaje.
        """
        try:
            ...
            for _, row in df.iterrows():
                ml_val = "Pendiente"
                if not pd.isna(row.get('prediction_label')):
                    ml_val = ("⚠️ Probable Falso Positivo"
                              if row['prediction_label'] == 0
                              else "🚨 Credencial Real")

                # confidence_score formateado como porcentaje (ej. "87.5%")
                ...
                values = (
                    ...
                    row['line_number'],
                    row['severity'],
                    conf_str,           # NUEVO: columna Confianza
                    row['commit_sha'][:7],
                    ml_val,
                    str(row.get('detection_date', ''))[:16],
                )
    ...
    def update_statistics(self):
        ...
        try:
            ...
            if not summary.empty:
                self.stats_labels['repos'].config(text=str(len(summary)))
                self.stats_labels['commits'].config(text=str(summary['commits_analyzed'].sum()))
                self.stats_labels['credentials'].config(text=str(summary['credentials_count'].sum()))
                avg_risk = summary['avg_risk_score'].mean()
                self.stats_labels['risk_level'].config(text=f"{avg_risk:.2f}" if avg_risk else "0")
    ...
    def render_charts(self, summary_df):
        """Genera y muestra los gráficos en el dashboard"""
        ...
        try:
            ...
            if not creds_df.empty and 'severity' in creds_df.columns:
                ...
                for t in texts:     t.set_color(self.colors['text'])
            ...
            tk.Label(self.left_chart_frame, text=f"⚠️ {e}",
                     bg=self.colors['bg_dark'], fg=self.colors['critical']).pack()
        ...
        try:
            ...
            if not filtered_df.empty:
                ...
                sns.barplot(data=filtered_df, x='repo_name', y='credentials_count',
                            ax=ax2, palette=[self.colors['accent']],
                            edgecolor=self.colors['text'])
                ax2.set_title('Detecciones por Proyecto', color=self.colors['accent'], pad=15)
                ...
                tk.Label(self.right_chart_frame, text="✅ No hay amenazas detectadas",
                         ...
                         font=('Segoe UI', 10, 'bold')).pack(pady=50)
        except Exception as e:
            tk.Label(self.right_chart_frame, text=f"⚠️ {e}",
                     bg=self.colors['bg_dark'], fg=self.colors['critical']).pack()
    ...
    def train_model(self):
        ...
        try:
            ...
            self.log("📡 Entrenando Isolation Forest (Anomalías)...")
            commits_df = db_manager.get_commits_dataframe()
            if not commits_df.empty:
                ml_anomaly_detector.train(commits_df)
                
            self.log("📡 Entrenando K-Means (Clustering de Riesgo)...")
            repos_df = db_manager.get_repository_summary()
            if not repos_df.empty:
                ml_repo_clusterer.train_and_predict(repos_df)
    ...
    def show_feature_importance(self):
        ...
        self.ml_detail_text.insert('end', "IMPORTANCIA DE CARACTERÍSTICAS (Gini):\n\n")
        for feature, importance in commit_classifier.feature_importance.items():
            bar = '█' * int(importance * 40)
            self.ml_detail_text.insert('end', f"{feature:30s}: {importance:.6f}  {bar}\n")
    ...
    def show_model_info(self):
        ...
        info = (
            ...
            f"Algoritmo: Árbol de Decisión CART\n"
            f"Criterio: Índice de Gini\n"
            f"Biblioteca: scikit-learn\n\n"
            f"Configuración:\n"
            f"  Max Depth:          {commit_classifier.config['max_depth']}\n"
            f"  Min Samples Split:  {commit_classifier.config['min_samples_split']}\n"
            f"  Random State:       {commit_classifier.config['random_state']}\n\n"
            f"Características: {len(commit_classifier.feature_names)}\n"
            f"{', '.join(commit_classifier.feature_names)}"
        )
    ...
    def show_about(self):
        ...
        messagebox.showinfo("Acerca de", (
            ...
            "Tecnologías: Python · PostgreSQL · scikit-learn · tkinter\n\n"
