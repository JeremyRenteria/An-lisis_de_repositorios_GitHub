...
from models.ml_credential_classifier import ml_credential_classifier
        ...
        for line_index, line in enumerate(lines):
            for credential_type, pattern in self.compiled_patterns.items():
                for match in pattern.finditer(line):
                    ...
                    # --- Filtro 4: ML confidence score ---
                    # En lugar de la eurística manual, usamos el modelo de ML para decidir
                    ml_result = ml_credential_classifier.predict_single(match_text, line, file_path)
                    
                    if not ml_result['is_credential'] and ml_result['confidence'] < self.min_confidence:
                        continue
        ...
        for line_index, line in enumerate(lines):
            ...
            for credential_type, pattern in self.compiled_patterns.items():
                for match in pattern.finditer(clean_line):
                    ...
                    # En lugar de la heurística, usamos ML
                    ml_result = ml_credential_classifier.predict_single(match_text, clean_line, file_path)
